...
ctx.strokeStyle = "hsla(" + getRandomInt(0, 360) + ",90%,60%,0.6)";
start();
body.removeEventListener("click", start, false);

// Alternatively, modify the start function to avoid stopping the animation
var start = () => {
    if (!stopped) return; // If already running, don't stop it
    stopped = false;
    requestAnimationFrame(animate);
};
